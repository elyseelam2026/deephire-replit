...
- **Profile URL Discovery for Team Discovery** (Oct 20, 2025): Revolutionary enhancement to Google Search fallback that discovers individual profile pages instead of extracting from snippets. Uses 14 URL pattern searches (e.g., `site:carlyle.com/team/`, `site:carlyle.com/about-carlyle/team/`, `site:carlyle.com/people/`, etc.) to find 100s of individual profile URLs. Grok AI extracts names directly from page titles (e.g., "William McMullan | Carlyle" → "William McMullan"). Successfully discovered 9+ members from Carlyle (Mark Blaufuss, Scott Jenkins, William McMullan, Sean Arnold, Dennis Wang, etc.) - **3x improvement over snippet approach**. No scraping required - extracts from titles/snippets only. Comprehensive pattern coverage: `/team/`, `/people/`, `/leadership/`, `/our-people/`, `/our-team/`, `/management/`, `/executives/`, company-specific patterns like `/about-carlyle/team/`. Smart filtering excludes listing pages, pagination, and filters to keep only individual profiles. System scales to any company size and bypasses Cloudflare completely by discovering URLs through Google.
