...
- **Hybrid Career History Extraction** (Oct 16, 2025): Implemented fallback system to handle Bright Data's career history censorship. When structured API returns censored data (asterisks), system automatically extracts career history from the AI-generated biography text instead. Uses Grok to parse biography and structure career data (company names, job titles, dates) even when raw LinkedIn data is masked. Provides reliable career history extraction regardless of API limitations.
